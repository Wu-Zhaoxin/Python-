...
def main():
    ...
    download(series_v, series_a, headers, proxies)
...
def get_page(baseurl):
    ...
    proxies = {
        "https": "https://112.6.117.135"
    }

    resp = requests.get(baseurl, headers=headers, proxies=proxies)
...
def download(series_v, series_a, headers, proxies):
    for one_series in series_v:
        resp = requests.get(one_series, headers=headers, proxies=proxies)
        with open(rf'video\{series_v.index(one_series) + 1}.mp4', mode="wb") as f:
            f.write(resp.content)
            print("写入第%d个视频文件" % series_v.index(one_series))
    for one_series in series_a:
        resp = requests.get(one_series, headers=headers, proxies=proxies)
        with open(rf'audio\{series_a.index(one_series) + 1}.mp3', mode="wb") as f:
            f.write(resp.content)
            print("写入第%d个音频文件" % series_a.index(one_series))
